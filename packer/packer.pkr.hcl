...
ssource "googlecompute" "gcp-cuda" {
  account_file        = var.account_file
  disk_size           = 50
  image_family        = var.image_family
  image_name          = "${var.image_name}-${local.timestamp}"
...
build {
  sources = ["source.googlecompute.gcp-cuda"]

  provisioner "ansible" {
    ansible_env_vars = ["ANSIBLE_HOST_KEY_CHECKING=False", "ANSIBLE_SSH_ARGS='-o ForwardAgent=yes -o ControlMaster=auto -o ControlPersist=60s -o PubkeyAcceptedKeyTypes=+ssh-rsa -o HostKeyAlgorithms=+ssh-rsa'"]
    extra_arguments  = ["-vvv", "--extra-vars", "@manifest/cuda-software.json", "--scp-extra-args", "'-O'"]
    playbook_file    = "./linux-cuda-playbook.yml"
  }
